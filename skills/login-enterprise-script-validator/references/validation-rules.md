...
> ScriptEditor** shipped (the build referenced when validating). This file documents the set as
> of `ScriptBase 3.1.611`; the validator always reflects the installed build, which is
> the source of truth.
...
- **Only compile-time-known arguments are checked.** If a timer name/value is not a literal the
  analyzer can resolve, it bails (`if (argumentValue is null) return;`). Dynamic names are not
  validated — so a script can pass validation and still have a runtime timer-name problem.
